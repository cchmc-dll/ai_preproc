...
class TIF_loader:
    def __init__(self, problem_type='Classification', input_images=None, input_shape=(128, 128), image_modalities=['T2'], mask=None, slice_number=0):
        ...
        if self.problem_type is 'Segmentation':
            ...
            self.truth_data_shape = tuple(
                [0, len(self.mask)] + list(self.input_shape))
    ...
    def load_toHDF5(self, hdf5_file=None, verbose=-1):
        ...
        image_storage = hdf5_file.create_earray(hdf5_file.root, 'imdata', tables.Float32Atom(
        ), shape=self.image_data_shape, filters=filters, expectedrows=n_samples)

        if self.problem_type is "Classification":
            truth_storage = hdf5_file.create_earray(hdf5_file.root, 'truth', tables.StringAtom(
                itemsize=15), shape=self.truth_data_shape, filters=filters, expectedrows=n_samples)
        elif self.problem_type is "Segmentation":
            truth_storage = hdf5_file.create_earray(hdf5_file.root, 'truth', tables.UInt8Atom(
            ...
            if self.problem_type is "Classification":
                ...
                if subject_name in self.ids:
                    images = self.get_images(
                        in_files=imagePath, image_shape=self.input_shape, label_indices=len(imagePath)-1)
                ...
                if subject_name in self.ids:
                    images = self.get_images(in_files=imagePath, image_shape=self.input_shape, label_indices=len(
                        imagePath)-1, slice_number=self.slice_number)
                    subject_data = [image for image in images]
