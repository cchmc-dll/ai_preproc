...
from .utils.utils import resize, read_image_files, read_image_files_wStats
...
def reslice_image_set(in_files, image_shape, out_files=None, label_indices=None, crop=False, stats=False):
    if crop:
        crop_slices = get_cropping_parameters([in_files])
    else:
        crop_slices = None
    if stats:
        images, imstats = read_image_files_wStats(
...
def get_volume(imstats, mask=None, label=0, units="mm"):
    """
    imstats: list of image statistics ()
    mask: data array of mask volume, could contain more than 1 label,
           if None calculates whole image volume
    label: label number for which volume should be calculated,
            if 0 calculates volume of all pixels except the minimum most pixel value
    units: "mm","cm" or "m". volume units, default "mm"
    return volume
    """
    volume = []
    if mask is None:
        volume = np.prod(imstats)
    else:
        if label > 0:
            # count voxels in the mask
            maskvoxels = (mask == label).sum()
        else:
            minval = mask.min()
            maskvoxels = (mask > minval).sum()
        # multiply the voxel volume by the voxels in the mask
        volume = np.prod(imstats[3:])*maskvoxels
...
def normalize_clinical_storage(data_storage):
    ...
    for index in range(data_storage.shape[0]):
        ...
        data_storage[index] = np.divide(data_storage[index], std, out=np.zeros_like(
            data_storage[index]), where=std != 0)
    return data_storage
...
def reslice_image_set_TIF(in_files, image_shape, out_files=None, label_indices=None, slice_number=0, use_middle_slice=False):
    ...
    for f, file in enumerate(in_files):
        ...
        if len(tiff.shape) > 2:
            image = tiff[len(tiff) //
                         2] if use_middle_slice else tiff[slice_number]
        else:
            tiff = io.imread(file)
            image = tiff
